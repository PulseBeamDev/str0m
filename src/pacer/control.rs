...
impl PacerControl {
    ...
    pub fn calculate(
        ...
        let min_pacing_rate = app_rate.max(estimate) * PACING_FACTOR;
...
mod test {
    ...
    fn padding_enabled_with_active_media() {
        ...
        let r = c.calculate(Bitrate::ZERO, true, estimate, false);
    ...
    fn no_padding_without_active_media() {
        ...
        let r = c.calculate(Bitrate::ZERO, false, estimate, false);
    ...
    fn overuse_suppresses_padding() {
        ...
        let r = c.calculate(Bitrate::ZERO, true, estimate, true);
        assert_eq!(r.padding_rate, Bitrate::ZERO);
    }

    #[test]
    fn app_rate_above_estimate_acts_as_pacing_floor() {
        let c = PacerControl::new();

        let r = c.calculate(Bitrate::kbps(2_000), true, Bitrate::kbps(500), false);

        assert_eq!(r.pacing_rate, Bitrate::kbps(2_000) * PACING_FACTOR);
    }

    #[test]
    fn app_rate_below_estimate_does_not_reduce_pacing() {
        let c = PacerControl::new();

        let r = c.calculate(Bitrate::kbps(500), true, Bitrate::kbps(2_000), false);
