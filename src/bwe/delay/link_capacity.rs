...
impl LinkCapacity {
    ...
    pub fn lower_bound(&self) -> Option<Bitrate> {
        let estimate = self.estimate_kbps?;
        let kbps = (estimate - 3.0 * self.deviation_estimate_kbps()).max(0.0);
        Some(Bitrate::bps((kbps * 1000.0) as u64))
    }
...
mod test {
    ...
    #[test]
    fn no_bounds_without_a_sample() {
        let capacity = LinkCapacity::new();
        assert!(!capacity.has_estimate());
        assert!(capacity.upper_bound().is_none());
        assert!(capacity.lower_bound().is_none());
    }

    #[test]
    fn first_sample_initialises_the_estimate() {
        let mut capacity = LinkCapacity::new();
    ...
    fn bounds_straddle_the_estimate() {
        ...
        let lower = capacity.lower_bound().expect("lower").as_f64();
